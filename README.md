...
Then import inside the query into your `gatsby-config.js` file and reference the export inside of the `query` field in the `graphcms` plugin options object.
...
A lot of the concepts and techniques we explored in the previous two sections will come in very handy here. What we'll need to do here is construct a GraphQL query in the `event.js` template file, selecting only the correct event listing data using the `id` variable that we gave the file access to via the `context` object in the `gatsby-node.js` file.

The only new concept in this section is retrieving a single event inside of the query via variables. The graphql query only has access to the variables that we pass down via the `context` object. We can utilise these variables like so:
...
The variables we pass to our queries need to be typed. The values we pass to the query via the `context` object are all prepended with the a `$` so we can differentiate them from a field. Here's an explanation as to what's going on in the line below (the numbers correspond with the code snippet below). We're asking GraphQL to send us back the `event` data (1) whose `id` field (2) equals (3) the `$id` variable we're given (4).
...
You should have enough familiarity with Gatsby, GraphQL and GraphCMS to finish the section of yourself. We're introducing now new concepts. The steps that you need to complete are as follows:
